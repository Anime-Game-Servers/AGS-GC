...
import org.anime_game_servers.multi_proto.gi.messages.scene.entity.TrifleGadget;
...
public class GadgetItemContent extends GadgetContent {
    ...
    public void onBuildProto(SceneGadgetInfo gadgetInfo) {
        val hostVersion = this.getGadget().getWorld().getHost().getSession().getVersion();
        val itemProto = item.toProto();
        ...
        if(hostVersion.getId() >= Version.GI_4_0_0.getId()){
            ...
            content = new SceneGadgetInfo.Content.TrifleItem(itemProto);
        }
